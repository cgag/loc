        ...
        Some(string) => match Sort::from_str(string) {
            ...
            Err(err) => {
                if let Some(suggestion) = err {
                    println!(
                        "Error: invalid value for --sort: '{}', perhaps you meant '{}'?",
                        string, suggestion
                    );
                } else {
                ...
                println!(
                    " Hint: legal values are Code, Comment, Blank, Lines, Language, \
                     and Files"
                );
                return;
            }
    ...
    if by_file {
        ...
        for (lang, mut filecounts) in by_lang {
            ...
            match sort {
                ...
                Sort::Language | Sort::Files => panic!(
                    "Sorting by language or files \
